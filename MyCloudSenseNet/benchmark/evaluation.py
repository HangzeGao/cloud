...
from benchmark.utils import mask2label
from benchmark.config import NUM_CLASSES, INVALID_LABEL_VALUE
...
def intersection_over_union_and_coverage(
    ...
    """
    Calculate Intersection over Union (IoU) and coverage metrics (legacy interface).
...
def read_prediction_and_aligned_true(
    ...
    """
    Read prediction and align it with ground truth data.

    Handles cases where prediction and ground truth have different resolutions
    or extents by using geospatial alignment.

    Args:
        data_path: Path to full image data with labels (bands 1-4=data, band 5=mask)
