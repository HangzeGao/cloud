...
# D4_TTA_MODES = ("none", "hflip", "vflip", "hvflip", "rot90", "rot180", "rot270")
#
#
...
#
#
# def geotiff_restore_metadata(
#         metadata_path,
#         original_img_path,
#         pred_mask_dir,
#         output_path,
#         fusion_method="vote",
#         probability_dir: Optional[os.PathLike] = None,
#         num_classes: int = 3,
# ):
#     if fusion_method == "probability":
#         prob_dir = probability_dir if probability_dir is not None else pred_mask_dir
#         return restore_probability_predictions(metadata_path, original_img_path, prob_dir, output_path, num_classes)
#
...
#
#
# def _resolve_device_type(model: CloudModel) -> str:
...
#
#
# def _prepare_model_for_inference(model: CloudModel) -> str:
#     device_type = _resolve_device_type(model)
...
#
#
# def _resolve_tta_modes(tta_modes: Union[bool, str, Sequence[str], None] = DEFAULT_TTA_MODES) -> Sequence[str]:
#     if tta_modes is None or tta_modes is False:
#         return ("none",)
#     if tta_modes is True:
...
#
#
# def _reverse_tta(x: torch.Tensor, mode: str) -> torch.Tensor:
#     if mode == "none":
#         return x
#     if mode == "hflip":
#         return torch.flip(x, dims=(-1,))
...
#         model: CloudModel,
#         x: torch.Tensor,
#         tta_modes: Union[bool, str, Sequence[str], None] = DEFAULT_TTA_MODES,
# ) -> torch.Tensor:
#     """Return class probabilities averaged over test-time augmentations."""
#     modes = _resolve_tta_modes(tta_modes)
#     probs = []
#
#     for mode in modes:
#         logits = model.forward(_apply_tta(x, mode))
#         prob = torch.softmax(logits, dim=1)
#         probs.append(_reverse_tta(prob, mode))
#
#     return torch.stack(probs, dim=0).mean(dim=0)
#
#
# def _prediction_profile(row: pd.Series, bands: Sequence[str]) -> Optional[dict]:
#     first_band_path = row.get(f"{bands[0]}_path")
#     if first_band_path is None:
#         return None
#
...
#
#
# def _save_mask_geotiff(path: Path, pred: np.ndarray, profile: Optional[dict] = None):
#     if profile is None:
...
#
#
# def _save_probability_geotiff(path: Path, prob: np.ndarray, profile: Optional[dict] = None):
#     if profile is None:
#         np.save(path.with_suffix(".npy"), prob.astype(np.float32))
#         return
#
#     profile.update(
#         count=prob.shape[0],
...
#     profile.pop("predictor", None)
#     with rasterio.open(path, "w", **profile) as dst:
#         dst.write(prob.astype(np.float32))
#
#
# def restore_probability_predictions(
#         metadata_path: os.PathLike,
#         original_img_path: os.PathLike,
#         pred_prob_dir: os.PathLike,
#         output_path: os.PathLike,
#         num_classes: int = 3,
# ):
#     """Restore overlapping chip probability GeoTIFFs into one full-scene mask."""
#     logger.info("Reading metadata and original image information")
#     df_meta = pd.read_csv(metadata_path)
#     pred_prob_dir = Path(pred_prob_dir)
#
#     with rasterio.open(original_img_path) as src:
#         orig_h, orig_w = src.height, src.width
#         orig_meta = src.meta.copy()
...
#         orig_transform = src.transform
#
#     prob_full = np.zeros((num_classes, orig_h, orig_w), dtype=np.float32)
#     weight_full = np.zeros((orig_h, orig_w), dtype=np.float32)
#
#     logger.info("Restoring probability chips to a large image")
#     for _, row in tqdm(df_meta.iterrows(), total=len(df_meta), desc="restoring"):
#         x1, y1 = int(row["x_start"]), int(row["y_start"])
#         x2, y2 = int(row["x_end"]), int(row["y_end"])
#         chip_prob_path = pred_prob_dir / f"{row['chip_id']}.tif"
#
#         with rasterio.open(chip_prob_path) as src:
#             chip_prob = src.read().astype(np.float32)
#
#         chip_h, chip_w = y2 - y1, x2 - x1
#         prob_full[:, y1:y2, x1:x2] += chip_prob[:, :chip_h, :chip_w]
#         weight_full[y1:y2, x1:x2] += 1
#
#     weight_full[weight_full == 0] = 1
...
#         "compress": "deflate",
#     })
#     orig_meta.pop("predictor", None)
#
#     with rasterio.open(output_path, "w", **orig_meta) as dst:
#         dst.write(pred_full, 1)
#
#     logger.info("Probability restoration completed!")
#
#
...
# ):
#     """
#     Predict chip masks with optional TTA probability averaging.
#
#     Args:
#         model: Loaded CloudModel.
#         x_paths: Chip metadata containing chip_id and band paths.
#         bands: Band names, used to read chip paths and output profiles.
#         predictions_dir: Directory for class-mask GeoTIFF outputs.
#         tta_modes: False/None disables TTA, True uses flips, "d4" adds rotations.
#         save_probabilities: Save softmax probability GeoTIFFs for probability fusion.
#         probability_dir: Optional output directory for probability GeoTIFFs.
#     """
#     predictions_dir = Path(predictions_dir)
#     predictions_dir.mkdir(exist_ok=True, parents=True)
#     if save_probabilities:
#         probability_dir = Path(probability_dir) if probability_dir else predictions_dir / "probabilities"
#         probability_dir.mkdir(exist_ok=True, parents=True)
#
#     device_type = _prepare_model_for_inference(model)
#
#     test_dataset = CloudDataset(x_paths=x_paths, bands=bands)
#     metadata_by_chip = x_paths.copy()
#     metadata_by_chip["chip_id"] = metadata_by_chip["chip_id"].astype(str)
#     metadata_by_chip = metadata_by_chip.set_index("chip_id", drop=False)
...
#             probs = predict_proba_with_tta(model, x, tta_modes=tta_modes)
#             preds = torch.argmax(probs, dim=1).detach().cpu().numpy().astype("uint8")
#             probs = probs.detach().cpu().numpy().astype("float32")
#
#             for chip_id, pred, prob in zip(batch["chip_id"], preds, probs):
#                 row = metadata_by_chip.loc[chip_id]
#                 profile = _prediction_profile(row, bands)
#                 chip_pred_path = predictions_dir / f"{chip_id}.tif"
#                 _save_mask_geotiff(chip_pred_path, pred, profile)
#
#                 if save_probabilities:
#                     chip_prob_path = probability_dir / f"{chip_id}.tif"
#                     _save_probability_geotiff(chip_prob_path, prob, profile)
#
#     return Path(probability_dir) if save_probabilities else None
#
#
# def predict_and_restore_geotiff(
#         model: CloudModel,
#         x_paths: pd.DataFrame,
#         metadata_path: os.PathLike,
#         original_img_path: os.PathLike,
...
# ):
#     """
#     Predict large-scene chips and restore them with probability fusion.
#
#     This is the preferred path for sliced remote-sensing scenes: TTA produces
#     per-chip softmax probabilities, then overlapping chips are averaged before
#     the final argmax.
#     """
#     if fast_dev_run:
#         x_paths = x_paths.head()
#
#     probability_dir = make_predictions(
#         model=model,
#         x_paths=x_paths,
#         bands=bands,
...
#     )
#     restore_probability_predictions(
#         metadata_path=metadata_path,
#         original_img_path=original_img_path,
#         pred_prob_dir=probability_dir,
