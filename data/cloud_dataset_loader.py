...
class RandomFlip:
    """随机翻转"""
    def __init__(self, horizontal=True, vertical=True, p=0.5):
...
def get_dataloader(
    ...
    **kwargs
    ...
    参数:
        ...
        batch_size: 批次大小
        num_workers: 数据加载线程数
        bands: 波段列表，默认 ["B02", "B03", "B04", "B08"]
        datasets: 筛选数据集列表，如 ["oncloudn", "cloud38"]
        normalize: 归一化策略:
            - "auto": 自动根据数据集选择 (默认)
            - "reflectance": TOA反射率归一化 (/10000)
            - "minmax": Min-Max归一化
            - "standard": 标准分数归一化 (基于典型统计值)
            - "bandwise_standard": 波段独立标准分数 (基于预计算统计)
            - "percentile": 1-99百分位数归一化
            - "none": 不归一化
        augmentation: 是否启用数据增强
        **kwargs: 其他传递给UnifiedCloudDataset的参数
    ...
    print(f"\n归一化配置:")
    print(f"  模式: {norm_info['mode']}")
    for ds, params in norm_info['dataset_params'].items():
